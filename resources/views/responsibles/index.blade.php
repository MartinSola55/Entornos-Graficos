...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div id="modalCreate" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
            <div id="formContainer" class="modal-dialog">
                <form role="form" class="needs-validation" method="POST" action="{{ url('/responsible/create') }}" id="form-create" autocomplete="off" novalidate>
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <div class="d-flex flex-row justify-content-between">
                            <h2 class="card-title">Listado de responsables</h2>
                            <button id="btnAdd" type="button" class="btn btn-info btn-rounded waves-effect waves-light m-t-10 float-right" data-toggle="modal" data-target="#modalCreate">Agregar nuevo responsable</button>
                        </div>
                            ...
                            <table id="DataTable" class="table table-bordered table-striped">
                                ...
                                <tbody id="table_body">
                                    @foreach ($responsibles as $responsible)
                                        <tr data-id="{{ $responsible->Person->id }}">
                                            ...
                                            <td class="d-flex flex-row justify-content-center">
                                                <button type='button' class='btn btn-outline-info btn-rounded btn-sm mr-2' onclick='edit({{ json_encode($responsible) }})' data-toggle="modal" data-target="#modalCreate"><i class="bi bi-pencil"></i></button>
                                                <button type='button' class='btn btn-danger btn-rounded btn-sm ml-2' onclick='deleteObj({{ $responsible->person->id }})'><i class='bi bi-trash3'></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
        ...
        <form id="form-delete" action="{{ url('/responsible/delete') }}" method="POST" style="display: none;">
            @csrf
            <input type="hidden" name="id" value="">
        </form>
    </div>
    ...
    <script>
        ...
        function deleteObj(id) {
            Swal.fire({
                title: "¿Seguro deseas eliminar este responsable?",
    ...
    <script>
        ...
        $("#btnAdd").on("click", function () {
            $("#modalTitle").text("Agregar responsable");
            $("#formContainer form").attr("action", "/responsible/create");
        ...
        function edit(entity) {
            ...
            $("#formContainer form").attr("action", "/responsible/edit");
