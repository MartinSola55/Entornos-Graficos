...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div id="modalCreate" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
            <div id="formContainer" class="modal-dialog">
                <form role="form" class="needs-validation" method="POST" action="{{ url('/teacher/create') }}" id="form-create" autocomplete="off" novalidate>
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <div class="d-flex flex-row justify-content-between">
                            ...
                            <button id="btnAdd" type="button" class="btn btn-info btn-rounded waves-effect waves-light m-t-10 float-right" data-toggle="modal" data-target="#modalCreate">Agregar nuevo docente</button>
                        </div>
                                    ...
                                    @foreach ($teachers as $teacher)
                                        <tr data-id="{{ $teacher->Person->id }}">
                                            ...
                                            <td class="d-flex flex-row justify-content-center">
                                                <button type='button' class='btn btn-outline-info btn-rounded btn-sm mr-2' onclick='edit({{ json_encode($teacher) }})' data-toggle="modal" data-target="#modalCreate"><i class="bi bi-pencil"></i></button>
                                                <button type='button' class='btn btn-danger btn-rounded btn-sm ml-2' onclick='deleteObj({{ $teacher->person->id }})'><i class='bi bi-trash3'></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
        ...
        <form id="form-delete" action="{{ url('/teacher/delete') }}" method="POST" style="display: none;">
            @csrf
            <input type="hidden" name="id" value="">
        </form>
    </div>
    ...
    <script>
        ...
        function deleteObj(id) {
            Swal.fire({
                title: "¿Seguro deseas eliminar este docente?",
    ...
    <script>
        ...
        $("#btnAdd").on("click", function () {
            $("#modalTitle").text("Agregar docente");
            $("#formContainer form").attr("action", "/teacher/create");
        ...
        function edit(entity) {
            ...
            $("#formContainer form").attr("action", "/teacher/edit");
    ...
    <script>
        $('#DataTable').DataTable({
            "language": {
                ...
                "sInfoFiltered": "(filtrado de _MAX_ docentes en total)",
                "emptyTable": 'No hay docentes que coincidan con la búsqueda',
